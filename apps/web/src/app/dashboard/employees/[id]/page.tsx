...
export default function EmployeeProfilePage() {
  ...
  async function updateEmployeeStatus(status: EmployeeStatus) {
    if (!employee) {
      return;
    }

    const confirmed = window.confirm(
      `Change ${employee.firstName} ${employee.lastName}'s status to ${status.replaceAll(
        '_',
        ' ',
      )}?`,
    );
    ...
    try {
      ...
      setSuccess(
        `${employee.firstName} ${employee.lastName}'s status was updated to ${status.replaceAll(
          '_',
          ' ',
        )}.`,
      );
    ...
    <DashboardShell activePage="employees">
      <div className="space-y-8">
        ...
        <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
          <StatCard
            title="Documents"
            value={String(normalDocuments.length)}
            helper="Employee files"
          ...
          <StatCard
            ...
            helper={`${pendingLeaveCount} pending requests`}
          ...
          <StatCard
            ...
            helper={`${attendanceRecords.length} records`}
            icon={<CalendarClock size={18} />}
          />

          <StatCard
            title="Performance"
            value={String(performance?.reviews.length ?? 0)}
            helper={`${performance?.goals.length ?? 0} active goals`}
          ...
          <aside className="space-y-6">
            ...
            <section className="border border-black/10 bg-white">
              <div className="border-b border-black/10 px-6 py-5">
                ...
                <p className="mt-1 text-sm text-gray-500">
                  Quickly update this employee’s employment status.
                </p>
              </div>
              ...
              <div className="grid gap-3 p-6">
                <LifecycleButton
                  ...
                  helper="Mark this employee as active."
                  icon={<UserCheck size={16} />}
                  loading={lifecycleAction === 'ACTIVE'}
                  disabled={
                    Boolean(lifecycleAction) ||
                    employee.employmentStatus === 'ACTIVE'
                  }
                  onClick={() => updateEmployeeStatus('ACTIVE')}
                />

                <LifecycleButton
                  label="Mark on leave"
                  helper="Set the employee status to on leave."
                  icon={<CalendarDays size={16} />}
                  loading={lifecycleAction === 'ON_LEAVE'}
                  disabled={
                    Boolean(lifecycleAction) ||
                    employee.employmentStatus === 'ON_LEAVE'
                  }
                  onClick={() => updateEmployeeStatus('ON_LEAVE')}
                />

                <LifecycleButton
                  label="Suspend employee"
                  helper="Temporarily suspend the employee profile."
                  icon={<Ban size={16} />}
                  loading={lifecycleAction === 'SUSPENDED'}
                  disabled={
                    Boolean(lifecycleAction) ||
                    employee.employmentStatus === 'SUSPENDED'
                  }
                  onClick={() => updateEmployeeStatus('SUSPENDED')}
                  danger
                />

                <LifecycleButton
                  label="Terminate employee"
                  helper="Mark employee as terminated and set today as end date."
                  icon={<UserMinus size={16} />}
                  loading={lifecycleAction === 'TERMINATED'}
                  disabled={
                    Boolean(lifecycleAction) ||
                    employee.employmentStatus === 'TERMINATED'
                  }
                  onClick={() => updateEmployeeStatus('TERMINATED')}
                  danger
                />

                <LifecycleButton
                  label="Mark inactive"
                  helper="Deactivate the employee profile."
                  icon={<Power size={16} />}
                  loading={lifecycleAction === 'INACTIVE'}
                  disabled={
                    Boolean(lifecycleAction) ||
                    employee.employmentStatus === 'INACTIVE'
                  }
                  onClick={() => updateEmployeeStatus('INACTIVE')}
                />
              </div>
            </section>
            ...
            <section className="border border-black/10 bg-white">
              ...
              <div className="space-y-4 p-6">
                ...
                <SummaryItem
                  ...
                  label="Location"
                  value={
                    [employee.city, employee.province, employee.country]
                      .filter(Boolean)
                      .join(', ') || 'Not set'
                  }
                />
          ...
          <div className="space-y-6">
            ...
            >
              ...
              (performance.reviews.length === 0 &&
                ...
                performance.developmentPlans.length === 0) ? (
                ...
                <div className="grid gap-4 md:grid-cols-3">
                  <PerformanceTile
                    ...
                    helper={
                      performance.reviews[0]?.cycle.name ??
                      'No completed review cycle'
                    }
                  />
                  <PerformanceTile
                    label="Goals"
                  ...
                  <PerformanceTile
                    label="Development"
                    value={String(performance.developmentPlans.length)}
                    helper="Active development actions"
                  />
                </div>
              )}
            </Panel>
            ...
            <Panel
              ...
              description="Latest employee documents excluding payslips."
              actionHref={`/dashboard/employees/${employee.id}/documents`}
            ...
            <Panel
              ...
              description="Latest payroll documents for this employee."
              actionHref="/dashboard/payslips"
            ...
            <Panel
              title="Leave history"
              description="Recent leave requests for this employee."
                      ...
                      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                        <div>
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            <span
                              className={`border px-2 py-1 text-[11px] font-medium ${getBadgeClass(
                                request.status,
                              )}`}
                            >
                          ...
                          <p className="mt-2 text-sm text-gray-500">
                            ...
                            {getLeaveDays(request)} day
                            {getLeaveDays(request) === 1 ? '' : 's'}
                          </p>
                        </div>
            ...
            <Panel
              ...
              description="Recent attendance records for this employee."
                    ...
                    >
                      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                        <div>
                          ...
                          <p className="mt-2 text-sm text-gray-500">
                            In {formatTime(record.clockInAt)} · Out{' '}
                            {formatTime(record.clockOutAt)} · Worked{' '}
                            {formatDuration(getDurationMinutes(record))}
                          </p>
                        </div>

                        <p className="text-xs text-gray-400">
                          Late by {record.lateByMinutes ?? 0} min
                        </p>
                      </div>
                    </article>
                  ))}
  ...
  return (
    <section className="border border-black/10 bg-white">
      <div className="flex flex-col gap-4 border-b border-black/10 px-6 py-5 md:flex-row md:items-center md:justify-between">
        ...
        <Link
          href={actionHref}
          className="inline-flex items-center justify-center border border-black/10 bg-white px-4 py-2 text-sm text-gray-600 transition hover:border-black hover:text-black"
        >
          {actionLabel}
        </Link>
      </div>
      ...
      <div>
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span className="border border-black/10 bg-white px-2 py-1 text-[11px] font-medium text-gray-500">
            {formatLeaveType(document.category)}
          </span>
...
function PerformanceTile({
  ...
  return (
    <article className="border border-black/10 bg-[#f8fafc] p-4">
      ...
      <p className="mt-3 text-xl font-semibold tracking-[-0.04em] text-[#111827]">
        {value}
      </p>
